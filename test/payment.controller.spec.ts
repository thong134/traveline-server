...
describe('PaymentController guards', () => {
  ...
  it('should block payout listing for other owners when not admin', () => {
    const call = () =>
      controller.listPayouts(2, { userId: 1, role: UserRole.User } as any);
  ...
  it('should allow payout listing for admin', () => {
    ...
    const result = controller.listPayouts(2, {
      userId: 1,
      role: UserRole.Admin,
    } as any);
  ...
  it('should block payout status update for non-admin', () => {
    const call = () =>
      controller.updatePayoutStatus(10, { status: PayoutStatus.PROCESSING }, {
        userId: 5,
        role: UserRole.User,
      } as any);
    expect(call).toThrow(ForbiddenException);

...
} from './restaurant-booking.entity';
...
export class RestaurantBookingsService {
  ...
  async create(dto: CreateRestaurantBookingDto): Promise<RestaurantBooking> {
  ...
  async findAll(
    ...
    if (params.userId) {
      qb.andWhere('booking.userId = :userId', { userId: params.userId });
    }
    if (params.tableId) {
  ...
  async findOne(id: number): Promise<RestaurantBooking> {

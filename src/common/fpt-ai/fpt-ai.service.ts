...
export class FptAiService {
  ...
  async recognizeIdCard(imageBuffer: Buffer, filename: string = 'image.jpg'): Promise<any> {
    try {
      ...
      if (response.data.errorCode !== 0) {
        ...
        throw new Error(response.data.errorMessage || 'Failed to recognize ID card');
      }
  ...
  async faceMatch(
    ...
    try {
      ...
      if (Buffer.isBuffer(file2)) {
         ...
         const response = await firstValueFrom(this.httpService.get(file2, { responseType: 'arraybuffer' }));
         form.append('file[]', Buffer.from(response.data), { filename: 'file2.jpg' });
      }
      ...
      if (response.data.code !== '200') {
         ...
         if (response.data.errorCode && response.data.errorCode !== 0) {
            this.logger.error(`FPT.AI FaceMatch Error: ${JSON.stringify(response.data)}`);
            throw new Error(response.data.errorMessage || 'Failed to match faces');
         }
      }
